...
class OwnLoadImages:  # for inference
    def __init__(self, image_list, img_size=640, stride=32):
        ...
        if self.video_flag[self.count]:
            ...
            if not ret_val:
                ...
                if self.count == self.nf:  # last video
                    ...
                    image = self.image_list[self.count]
                    self.new_video(image)
                    ret_val, img0 = self.cap.read()

            self.frame += 1
            print(f'video {self.count + 1}/{self.nf} ({self.frame}/{self.nframes}) {image}: ', end='')
