...
model = init_detector(
        ...
        cfg_options=None,)
cfg = model.cfg
print("finish init mcgaze")


def gaze_det(bboxes_data):
  frame_id = 0
  person_num = 0
  video_clip=None
  video_clip_set = []
  ...
  while frame_id < vid_len:
      ...
      # f = open(txt_path, 'r')
      if str(frame_id) not in bboxes_data:
        frame_id += 1
        continue
      f = bboxes_data[str(frame_id)]
      #遍历每一行
      face_bbox = []
      for line in f.split("\n")[:-1]:
          line = line.strip()
  ...
  for vid_clip in video_clip_set:
      for i,frame_id in enumerate(vid_clip['frame_id']):  # 遍历每一帧
          ...
          for j in range(vid_clip['person_num']):  # 遍历每一个人
              ...
              bbox_area = (head_bboxes[1] - head_bboxes[3]) * (head_bboxes[0] - head_bboxes[2])
